...
def Aug_K(name):
    augments = {
    'TFT6_Augment_PandorasItems' : '판도라의 아이템',
...
def update(summonername):
    ...
    if user_obj:
        user.objects.filter(name=name).update(Level=level,profile_img=profileid)
    else:
        u.save()
    ...
    stat_obj = static.objects.filter(Name=name)
    if stat_obj:
        static.objects.filter(Name=name).update(Level=level,Tier=tier,LP=LP)
